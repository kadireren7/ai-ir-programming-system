...
| `TORQA_ALLOW_PREVIEW_FALLBACK` | Recommended in prod | Allow fallback from real provider to preview (`true`/`false`; default false in production) |
...
- **Schedule automation:** **Run now** and **cron tick** execute using the same provider path; cron is intentionally simple MVP worker-style execution.
...
| `/api/scan-schedules/cron/tick` | **POST** — `Authorization: Bearer $TORQA_CRON_SECRET`; executes due enabled schedules and returns run diagnostics |
...
- **`hosted-python`:** Set **`TORQA_SCAN_PROVIDER=hosted-python`** and **`TORQA_ENGINE_URL`** to the base URL of your Torqa Python HTTP service (no trailing slash required). The dashboard **`POST`s `{ source, content }` to `{TORQA_ENGINE_URL}/scan`** and maps the JSON body into the same **`ScanApiSuccess`** shape with explicit trust metadata (`engine_mode`, `analysis_kind`, `fallback`). Control downgrade behavior with **`TORQA_ALLOW_PREVIEW_FALLBACK`**: in production, keep this `false` to fail closed when real engine calls fail.
...
- **Scheduled scans foundation:** `/schedules` stores **`scan_schedules`** + **`scan_schedule_runs`**. **Run now** executes on the server (same engine as **`POST /api/scan`**) and persists results. **`POST /api/scan-schedules/cron/tick`** runs due enabled schedules when called with `TORQA_CRON_SECRET`. Integration-scoped schedules can be created, but execution returns a clear “not implemented” response until ingestion exists.
