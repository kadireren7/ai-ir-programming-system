...
export async function POST(request: Request) {
  ...
  const name   = typeof body.name   === "string" && body.name.trim() ? body.name.trim().slice(0, 120) : "n8n";

  if (!baseUrl) return NextResponse.json({ error: "Base URL must start with http:// or https://" }, { status: 400 });
  if (!apiKey)  return NextResponse.json({ error: "API key is required" }, { status: 400 });

  let encryptedToken: string;
  try {
    encryptedToken = encryptToken(apiKey);
  } catch {
  ...
  const { data: tokenRow, error: tokenErr } = await admin
    ...
    .insert({ user_id: user.id, provider: "n8n", token_type: "api_key", token_hint: hint, encrypted_token: encryptedToken })
    .select("id")
    .single();
  ...
  const integrationPayload = {
    ...
    config: { baseUrl, apiKeyMask: hint },
  };
  ...
  if (existing) {
    ...
    const { data, error } = await admin
      .from("integrations")
      .insert({ user_id: user.id, organization_id: null, provider: "n8n", name, ...integrationPayload })
      .select("id")
      .single();
  ...
  void logWorkspaceActivity(supabase, orgId, "integration.connected", integrationId, { provider: "n8n", baseUrl }).catch(() => {});
  void syncIntegration(user.id, integrationId).catch(() => {});

  return NextResponse.json({ ok: true, integrationId, apiKeyMask: hint, baseUrl });
}
