...
import { CheckCircle2, Loader2, X, XCircle, Workflow } from "lucide-react";
...
export function N8nConnectPanel({ open, onClose, existingBaseUrl, existingApiKeyMask, onDisconnect, onSaved }: Props) {
  const isManage = Boolean(existingBaseUrl && existingApiKeyMask);
  ...
  const handleSave = async () => {
    ...
    setSaveError(null);
    try {
      const res = await fetch("/api/integrations/n8n/connect", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ baseUrl, apiKey, name }),
      });
      const j = (await res.json()) as { ok?: boolean; error?: string };
      if (!res.ok || !j.ok) {
  ...
  const canTest = baseUrl.startsWith("http") && apiKey.length > 4;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {mode === "manage" ? "Manage connection" : "Connect your instance"}
              </p>
            </div>
          </div>
          ...
          {mode === "manage" ? (
            <>
              <div className="rounded-lg border border-border/50 bg-muted/20 px-4 py-3 space-y-2">
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium">Instance URL</p>
                  <p className="mt-0.5 text-sm font-mono">{existingBaseUrl}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium">API Key</p>
                  <p className="mt-0.5 text-sm font-mono tracking-widest text-muted-foreground">{existingApiKeyMask}</p>
                </div>
              </div>
              <Button
                variant="outline"
                size="sm"
                className="w-full"
                onClick={() => { setMode("form"); setBaseUrl(existingBaseUrl ?? ""); setApiKey(""); }}
              >
            ...
            <>
              ...
              <div className="space-y-2">
                <Label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Instance URL</Label>
                <Input
                  value={baseUrl}
                  onChange={(e) => { setBaseUrl(e.target.value); if (mode !== "form") setMode("form"); }}
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Settings → API in n8n. Only an encrypted hint is stored.</p>
                <Input
                  value={apiKey}
              ...
              <div className="flex gap-2">
                {mode !== "tested_ok" ? (
                  ...
                  <Button
                    size="sm"
                    disabled={mode === ("saving" as PanelState)}
