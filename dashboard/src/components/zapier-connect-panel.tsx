...
export function ZapierConnectPanel({ open, onClose, existingMask, existingAuthMethod, onDisconnect, onSaved }: Props) {
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-border/60 bg-background shadow-2xl">
          ...
          {mode === "manage" ? (
            <>
              <div className="rounded-lg border border-border/50 bg-muted/20 px-4 py-3 space-y-2">
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium">Auth method</p>
                  <p className="mt-0.5 text-sm capitalize">{existingAuthMethod ?? "apikey"}</p>
                </div>
                <div>
            ...
            <>
              ...
              <div className="rounded-lg border border-border/50 bg-muted/10 px-4 py-3 space-y-2">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">OAuth (recommended)</p>
                <p className="text-xs text-muted-foreground">Connect securely via Zapier OAuth — no API key needed.</p>
                <Button
                  variant="outline"
                  size="sm"
                  className="w-full"
                  onClick={() => { window.location.assign("/api/integrations/zapier/oauth/start"); }}
                >
                  Connect via OAuth
                </Button>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. My Zapier" disabled={mode === "testing" || mode === "saving"} />
              </div>
              <div className="space-y-2">
                <Label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Personal Access Token</Label>
                <p className="text-xs text-muted-foreground">Zapier → Settings → Developer API → Generate token.</p>
                <Input
                  value={apiKey}
        ...
        <div className="border-t border-border/40 px-6 py-3">
          <a href="https://zapier.com/developer" target="_blank" rel="noopener noreferrer" className="text-xs text-muted-foreground hover:text-foreground">
