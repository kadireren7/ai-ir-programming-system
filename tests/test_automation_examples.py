"""examples/automation/*.tq — minimal step chain + gated emit must parse, validate, materialize."""
...
def test_automation_examples_parse_validate_and_materialize(tmp_path: Path) -> None:
    root = REPO / "examples" / "automation"
    tq_files = sorted(root.glob("*.tq"))
    assert tq_files, "expected .tq under examples/automation"
