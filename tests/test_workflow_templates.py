"""examples/workflows/*.tq — reusable workflow starters must parse, validate, and materialize."""
...
def test_workflow_templates_parse_validate_and_materialize(tmp_path: Path) -> None:
    root = REPO / "examples" / "workflows"
    tq_files = sorted(root.glob("*.tq"))
    assert tq_files, "expected .tq under examples/workflows"
